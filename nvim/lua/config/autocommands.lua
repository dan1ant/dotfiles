...
vim.api.nvim_create_autocmd('LspAttach', {
  ...
  callback = function(event)
    ...
    if client:supports_method(vim.lsp.protocol.Methods.textDocument_definition) then
      vim.keymap.set('n', 'gd', fzf.lsp_definitions, { desc = '(LSP) Definitions', buffer = bufnr })
    end

    if client:supports_method(vim.lsp.protocol.Methods.textDocument_declaration) then
      vim.keymap.set('n', 'gD', fzf.lsp_declarations, { desc = '(LSP) Declarations', buffer = bufnr })
    end

    if client:supports_method(vim.lsp.protocol.Methods.textDocument_references) then
      vim.keymap.set('n', 'gr', fzf.lsp_references, { desc = '(LSP) References', buffer = bufnr })
    end

    if client:supports_method(vim.lsp.protocol.Methods.textDocument_implementation) then
      vim.keymap.set('n', 'gI', fzf.lsp_implementations, { desc = '(LSP) Implementations', buffer = bufnr })
    end

    if client:supports_method(vim.lsp.protocol.Methods.textDocument_typeDefinition) then
      vim.keymap.set('n', '<leader>D', fzf.lsp_typedefs, { desc = '(LSP) Type Definitions', buffer = bufnr })
    end

    if client:supports_method(vim.lsp.protocol.Methods.textDocument_documentSymbol) then
      vim.keymap.set('n', '<leader>ds', fzf.lsp_document_symbols, { desc = '(LSP) Document Symbols', buffer = bufnr })
    end

    if client:supports_method(vim.lsp.protocol.Methods.workspace_symbol) then
      vim.keymap.set('n', '<leader>ws', fzf.lsp_workspace_symbols, { desc = '(LSP) Workspace Symbols', buffer = bufnr })
    end

    if client:supports_method(vim.lsp.protocol.Methods.textDocument_signatureHelp) then
      vim.keymap.set('i', '<C-S>', vim.lsp.buf.signature_help, { desc = '(LSP) Signature Help', buffer = bufnr })
    end
